...
export default function DayTour() {
  ...
  useEffect(() => {
    // ✅ Fix: filter server-side by tour_type to avoid fetching all tours
    fetch(`${API_BASE}/api/tours?tour_type=day`)
      .then(res => {
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto' }}>
        ...
        {error ? (
          <p style={{ textAlign: 'center', color: '#ef4444', fontWeight: 600, padding: '40px 0' }}>
            Failed to load day tours. Please refresh the page.
          </p>
        ) : loading ? (
          ...
          <p style={{ textAlign: 'center', color: '#9ca3af', padding: '40px 0' }}>No day tours available.</p>
        ) : (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 32 }}>
            {tours.map(tour => (
