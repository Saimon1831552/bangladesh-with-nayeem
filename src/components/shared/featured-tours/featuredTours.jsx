...
// ✅ Fix: env var instead of hardcoded URL
...
export default function FeaturedTours() {
  ...
  useEffect(() => {
    // ✅ Fix: error handling + loading state
  ...
  // ✅ Fix: isFeatured from DB is 0/1 integer, not boolean — use Number coercion
  const featured = tours.filter(t => Number(t.isFeatured) === 1);
  ...
  return (
    <div style={{ background: '#f8f6f1', padding: '72px 32px 80px' }}>
      <div style={{ maxWidth: 1280, margin: '0 auto' }}>
        ...
        {error ? (
          <p style={{ textAlign: 'center', color: '#ef4444', fontWeight: 600, padding: '40px 0' }}>
            Failed to load tours. Please refresh the page.
          </p>
        ) : loading ? (
          ...
          <p style={{ textAlign: 'center', color: '#9ca3af', padding: '40px 0' }}>No featured tours available.</p>
        ) : (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 32 }}>
            {featured.map(tour => (
