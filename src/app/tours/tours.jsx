...
// ── Global styles (injected once, not per card) ───────────────────────────────
const CARD_STYLES = `
  ...
  .tour-card:hover .cta-btn { background: #15803d !important; transform: translateY(-1px); box-shadow: 0 8px 24px rgba(22,163,74,0.35); }
...
async function fetchTours(filters = {}) {
  ...
  const url = `${API_BASE}/api/tours?${params.toString()}`;
...
export function TourCard({ tour }) {
  ...
  const [imgSrc, setImgSrc] = useState(tour.image_url || FALLBACK_IMG);
  ...
  // Build tour detail href safely
  const tourHref = tour.slug ? `/tours/${tour.slug.trim()}` : '#';
    ...
    >
      ...
      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: 20, zIndex: 10 }}>
        ...
        <h3 className="card-title" style={{
          ...
          display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden', backgroundColor: 'rgba(0,0,0,0.6)', padding: '4px 8px', borderRadius: 8,
        }}>
          {tour.title}
        </h3>
        ...
        <div style={{ display: 'flex', gap: 14, marginBottom: 16, flexWrap: 'wrap' }}>
          {tour.duration && (
            <span style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 12, color: 'rgba(255,255,255,0.7)', fontWeight: 600 }}>
        ...
        <Link
          ...
          style={{
            ...
            fontSize: 14, fontWeight: 700, textDecoration: 'none', overflow: 'hidden',
          }}
        >
          View Tour Details
...
export default function Tour() {
  ...
  return (
    <>
      {/* ✅ Fix 5: Styles injected ONCE at page level, not inside every card */}
          ...
          {error ? (
            ...
            <div style={{ textAlign: 'center', padding: '80px 24px' }}>
              ...
              <button
                ...
                style={{ marginTop: 16, padding: '10px 24px', borderRadius: 12, background: '#16a34a', color: '#fff', border: 'none', fontWeight: 700, cursor: 'pointer' }}
              >
                Clear filters
              </button>
            </div>
