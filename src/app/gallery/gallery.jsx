...
import {
  ...
  faImages, faExpand, faTriangleExclamation,
} from '@fortawesome/free-solid-svg-icons';

const API_BASE = (process.env.NEXT_PUBLIC_API_URL || 'https://api.bangladeshwithnaim.com').replace(/\/api\/?$/, '');

const FALLBACK_IMG = 'https://images.unsplash.com/photo-1587222318667-31212ce2828d?q=80&w=1400&auto=format&fit=crop';

function GalleryCell({ img, idx, height, isLast, extraCount, onOpen, onOpenFull }) {
  ...
  return (
    <div
      ...
      onClick={() => (isLast && extraCount > 0 ? onOpenFull() : onOpen(idx))}
    >
      <img
        src={src}
        alt={img.label}
  ...
  return (
    <div className="gl-row" style={{ gridTemplateColumns: 'repeat(6, 1fr)' }}>
      {Array.from({ length: 6 }).map((_, i) => (
        <div key={i} style={{
          height: 380, borderRadius: 18,
          background: 'linear-gradient(90deg,#141410 25%,#1c1c17 50%,#141410 75%)',
          backgroundSize: '200% 100%',
          animation: 'glSkShimmer 1.4s ease infinite',
        }} />
      ))}
    </div>
  );
...
function GalleryError({ onRetry }) {
  return (
    <div style={{
      ...
      <FontAwesomeIcon icon={faTriangleExclamation} style={{ fontSize: 32, color: '#f59e0b', marginBottom: 14 }} />
      <p style={{ fontSize: 15, color: 'rgba(255,255,255,0.7)', fontWeight: 600, marginBottom: 6 }}>Couldn't load the gallery</p>
      <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.35)', marginBottom: 20 }}>Please check your connection and try again.</p>
      <button
        onClick={onRetry}
        style={{
          padding: '10px 24px', borderRadius: 99, background: '#fff', color: '#090907',
...
export default function Gallery({ images: initialImages }) {
  ...
  const [images, setImages]   = useState(hasInitial ? initialImages : []);
  ...
  const navLb    = useCallback(d   => setLightboxIdx(p => (p + d + images.length) % images.length), [images.length]);
  ...
  return (
    <>
      <style>{`
        ...
        @keyframes glSkShimmer { 0%{background-position:200% 0} 100%{background-position:-200% 0} }
        ...
        .gl-empty { text-align:center; padding:80px 24px; color:rgba(255,255,255,0.35); font-size:14px; }
      ...
      <section className="gl">
        <div className="gl-in">
          ...
          <div className="gl-hd">
            ...
            <div className="gl-sc">
              {[
                { n: images.length ? `${images.length}+` : '—', l: 'Photos' },
                { n: '4.9★', l: 'Avg Rating' },
                { n: '100%', l: 'Private' },
              ].map((s, i) => (
          ...
          <div className="gl-rows">
            {error ? (
              <GalleryError onRetry={load} />
            ) : loading ? (
              <GallerySkeleton />
            ) : images.length === 0 ? (
              <p className="gl-empty">No gallery photos yet. Check back soon!</p>
            ) : (
              <div className="gl-row" style={{ gridTemplateColumns: 'repeat(6, 1fr)' }}>
                {PREVIEW.map((img, idx) => (
                  <GalleryCell
          ...
          {!loading && !error && images.length > 0 && (
            <div className="gl-ft">
              <div className="gl-ft-note">Showing <b>{PREVIEW.length}</b> of <b>{images.length}</b> photos</div>
              <div className="gl-fbtns">
                ...
                <a
                  className="gl-btn-g"
                  href="https://www.instagram.com/bangladesh.with.naim/"
                  target="_blank"
        ...
        <div className="fg">
          <div className="fg-hd">
            <div>
              ...
              <div className="fg-meta">{images.length} photos · click any image to expand</div>
            </div>
            ...
            <div className="fg-mason">
              {images.map((img, i) => (
                <div key={i} className="fg-itm" onClick={() => fromFull(i)}>
                  <img src={img.src} alt={img.label} loading="lazy" onError={e => { e.target.src = FALLBACK_IMG; }} />
                  <div className="fg-ihov">
                    <div className="fg-ilbl">{img.label}</div>
                    {img.country && <div className="fg-icnt">📍 {img.country}</div>}
                  </div>
      ...
      {lightboxIdx !== null && images[lightboxIdx] && (
        <div className="lb" onClick={closeLb}>
          <div className="lb-top">
            <div className="lb-ctr">{lightboxIdx + 1} / {images.length}</div>
          ...
          <div className="lb-bot" onClick={e => e.stopPropagation()}>
            <div className="lb-lrow">
              <span className="lb-lbl">{images[lightboxIdx].label}</span>
              {images[lightboxIdx].country && <span className="lb-lcnt">· {images[lightboxIdx].country}</span>}
            </div>
